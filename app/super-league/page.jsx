...
const SuperLeague = () => {
    ...
    const fetchSuperLeaguesByUser = async (userId) => {
        ...
        try {
            ...
            const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/superleague?user=${userId}`);
        ...
        <div className={`min-h-[88vh] flex flex-col my-8 text-white px-4 sm:px-8 md:px-10 lg:px-16 xl:px-20 pb-10 ${exo2.className}`}>
            <div className="flex justify-between items-center mb-8">
                <div className='flex items-center space-x-4'>
                    ...
                    <h1 className="text-4xl font-bold">{view === 'detail' ? "Super League" : view === 'create' ? "Create Super League" : "Super Leagues"}</h1>
                </div>
            ...
            {loading ? (
                ...
                <>
                        ...
                        showEmptyView ?
                            <div className="w-full relative custom-dash-spacing px-4 rounded-3xl shadow-lg flex flex-col items-center space-y-12 py-36 lg:py-16 xl:py-24 2xl:py-36">
                                <div className="flex flex-col items-center justify-center">
                                    <img src="/images/empty-super-league.png" alt="No super leagues" className="w-40 mb-5" />
                                    <h2 className="text-2xl font-bold mb-2">No Super Leagues Yet</h2>
                                    <p className="text-gray-300 mb-4">Create a Super League to track multiple leagues in one place.</p>
                                    <button
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                                {superLeagues.map((sl) => (
                                    <div key={sl._id} onClick={() => handleCardClick(sl)} className="group">
                                        <div className="rounded-2xl bg-white/10 hover:scale-105 transition-transform flex flex-col items-center p-6 cursor-pointer">
                    ...
                    {view === 'detail' && selectedSuperLeague && (
                        <SuperLeagueDetail superLeague={selectedSuperLeague} onBack={handleBackToList} />
                    )}
                    {view === 'create' && (
                        <SuperLeagueCreateForm User={user} onBack={handleBackToList} onCreated={handleBackToList} />
                    )}
                </>
            )}
            {showDeletePopup && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 ">
                    <div className={`bg-gradient-to-r from-[#0C1922] to-[#0C192250] backdrop-blur-sm p-6 rounded-xl shadow-md shadow-[#1f1f1f] text-center max-w-96 w-[90vw] ${exo2.className}`}>
                        ...
                        <p className="text-sm xl:text-base mb-4 text-gray-400">
                            Type <span className="font-bold text-white">{selectedSuperLeague?.name}</span> to confirm:
                        </p>
                        <input
                            type="text"
                        ...
                        />
                        {inputError && <p className="text-sm xl:text-base text-[#ca3c3c] mb-1">Input does not match.</p>}
                        <div className="flex justify-between mt-4">
                            <button
                                className={`px-4 sm:px-6 xl:px-8 py-1 md:py-2 text-sm xl:text-base rounded-xl shadow-md ${!inputSuperLeagueName ? "fade-gradient-no-hover opacity-50 cursor-not-allowed" : "fade-gradient"}`}
